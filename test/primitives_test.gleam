...
pub fn encode_false_test() {
  toon_codec.encode(Bool(False))
  |> should.equal("false")
}

pub fn decode_true_test() {
  toon_codec.decode("true")
  |> should.be_ok
  |> should.equal(Bool(True))
}

pub fn decode_false_test() {
  toon_codec.decode("false")
  |> should.be_ok
...
pub fn encode_integer_test() {
  toon_codec.encode(Number(42.0))
  |> should.equal("42")
}

pub fn encode_negative_integer_test() {
  toon_codec.encode(Number(-123.0))
  |> should.equal("-123")
}

pub fn encode_zero_test() {
  toon_codec.encode(Number(0.0))
  |> should.equal("0")
}

pub fn encode_decimal_test() {
  toon_codec.encode(Number(3.14))
  |> should.equal("3.14")
}

pub fn encode_negative_decimal_test() {
  toon_codec.encode(Number(-2.5))
  |> should.equal("-2.5")
}
...
pub fn decode_invalid_escape_sequence_test() {
  toon_codec.decode("\"invalid\\xescape\"")
  |> should.be_error
}
